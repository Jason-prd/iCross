...
class DropshippingService:
    ...
    def get_settings(self) -> DropshipSettings:
        settings = self.session.query(DropshipSettings).filter_by(team_id=self.team_id).first()
        if not settings:
    ...
    def get_spu_link(self, sku: str) -> Optional[ProductSupplierLink]:
        ...
        link = self.session.query(ProductSupplierLink).filter(
    ...
    def get_product_info(self, sku: str) -> Optional[Dict[str, Any]]:
        ...
        platform_product = self.session.query(PlatformProduct).filter_by(
        ...
        info = {
            ...
            "price": variant.price,
            "cost": variant.cost,
            "ozon_product_id": platform_product.platform_product_id if platform_product else None,
    ...
    def link_spu_to_supplier(self, sku: str, supplier_id: str, supplier_product_url: str,
                            purchase_cost: float = 0, shipping_cost: float = 0,
                            warehouse: str = "", is_preferred: bool = True) -> bool:
        spu = extract_spu(sku)
        
        variants = self.session.query(ProductVariant).filter(
            ProductVariant.sku.like(f"SKU#{spu[4:]}%")
        ).all()
        ...
        existing_link = self.session.query(ProductSupplierLink).filter_by(
            spu=spu,
    ...
    def get_all_spu_links(self) -> List[Dict[str, Any]]:
        links = self.session.query(ProductSupplierLink).filter_by(
            team_id=self.team_id,
            is_active=True
        ).all()
    ...
    def get_dashboard_stats(self) -> Dict[str, Any]:
        total_spus = self.session.query(ProductSupplierLink).filter_by(
            team_id=self.team_id, is_active=True
        ).count()
        
        total_orders = self.session.query(DropshipOrder).filter_by(team_id=self.team_id).count()
        pending = self.session.query(DropshipOrder).filter_by(team_id=self.team_id, status="pending").count()
        processing = self.session.query(DropshipOrder).filter_by(team_id=self.team_id, status="processing").count()
        shipped = self.session.query(DropshipOrder).filter_by(team_id=self.team_id, status="shipped").count()
    ...
    def import_from_excel(self, products_data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for product_info in products_data:
            ...
            success = self.link_spu_to_supplier(
                ...
                purchase_cost=purchase_cost,
                shipping_cost=shipping_cost,
                warehouse=warehouse if warehouse else "深圳",
                is_preferred=True
            )
